...
grid = Entity(model=Grid(20,20),
               scale=50,
               color=color.white,
               rotation_x=90,
               y = -1,
               collider = 'box')
water = Entity(model= Plane(subdivisions=(2,8)),
               scale=50,
               color=color.white,
               texture='water.png',
                rotation_x=0, y = -0.5)
...
fpc = FirstPersonController(model='cube',
                            collider = 'box', 
                            position=(0.5,1,0.5))
...
def end_game(user_message):
    message = Text(text = user_message, 
                   scale=2, 
                   origin=(0,0), 
                   background=True, 
                   color=color.blue)
